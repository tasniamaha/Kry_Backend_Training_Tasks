...
const args=process.argv.slice(2) //for removing first 2 element of process.argv which are command and script path
...
if(command=='join'){
    if( args.length<3){
        console.log('Not enough arguments,should be atleast 3') //join+2 path segment
        process.exit(1)
    }
    ...
    if( args.length!=2){
        console.error('Should atleast have one filepath')
        process.exit(1)
    }
